...
function initSoundsGame() {
    ...
    showSoundOptions();
    // Hayvan ismini göster
    showAnimalQuestion();
}

function showAnimalQuestion() {
    const animal = soundAnimals[currentAnimal];
    const questionDiv = document.querySelector('#soundsGame .sound-question h3');
    questionDiv.textContent = `Bu hayvan: ${animal.emoji}`;
}

function playCurrentSound() {
    // Ses kapalı - sadece hayvanı göster
    const animal = soundAnimals[currentAnimal];
    const questionDiv = document.querySelector('#soundsGame .sound-question h3');
    questionDiv.textContent = `Bu hayvan: ${animal.emoji}`;
}
...
function checkSoundAnswer(animal) {
    if (animal.name === soundAnimals[currentAnimal].name) {
        ...
        showFeedback('soundsGame', '🎉 Doğru! Çok iyi!', true);
